...
public class FlightCrewMemberDashboardShowService extends AbstractGuiService<FlightCrewMember, FlightCrewMemberDashboard> {
	...
	public void load() {
		...
		Map<AssignmentStatus, Integer> faByStatus = new HashMap<>();
	...
	public void unbind(final FlightCrewMemberDashboard dashboard) {
		...
		dataset.put("CONFIRMED", dashboard.getFlightAssignmentsGroupedByStatus().getOrDefault(AssignmentStatus.CONFIRMED, 0));
		dataset.put("PENDING", dashboard.getFlightAssignmentsGroupedByStatus().getOrDefault(AssignmentStatus.PENDING, 0));
		dataset.put("CANCELLED", dashboard.getFlightAssignmentsGroupedByStatus().getOrDefault(AssignmentStatus.CANCELLED, 0));
