...
public interface AirlineManagerDashboardRepository extends AbstractRepository {
	...
	@Query("select 65 - (year(:currentDate) - year(m.birthdate)) from AirlineManager m where m.id = :managerId")
	...
	Double findRatioStatusLegs(int managerId, Leg.Status status);
